...
async def pfc_start(ctx, name_player1: discord.Member = None, name_player2: discord.Member = None,
                    numberBestOf: int = 3):
    if numberBestOf > 5:
        await ctx.send("STOP, le nombre de partie est trop élevé voyons !")
    elif numberBestOf <= 0:
        await ctx.send(f"Super ! Faites {numberBestOf} parties ça va être drôle !")
    elif name_player1 == None or name_player2 == None:
        await ctx.send("Attention, vous devez specifier au moins 2 joueurs !")
    else:
        random.seed(None)
        i = 0
        player1 = p.Player(name_player1.display_name)
        player2 = p.Player(name_player2.display_name)

        await ctx.send(f"{player1} VS {player2}")
        while (player1.score + player2.score) < numberBestOf or (player1.score == player2.score):
            p.updateRound(player1, player2)

            await ctx.send(f"```Round {i + 1} :\n" f"{player1.current_plays()}\n"
                           f"{player2.current_plays()}\n"
                           f"{p.display_score(player1, player2)}```")
            i += 1
